...
export function NewTripCreation() {
  ...
  const handleGenerate = async (data?: TripFormData) => {
    ...
    try {
      // Build preference metadata if tracking exists
      let preferenceMetadata = undefined;
      if (tripData.preferenceTracking) {
        const tracking = tripData.preferenceTracking;
        const trackingValues = Object.values(tracking);
        
        preferenceMetadata = {
          totalPreferencesApplied: trackingValues.length,
          profilePreferences: trackingValues.filter((t: any) => t.source === 'profile').length,
          overriddenPreferences: trackingValues.filter((t: any) => t.source === 'override').length,
          defaultPreferences: trackingValues.filter((t: any) => t.source === 'default').length,
          tracking
        };
      }
