...
import { PreferenceOverride } from '../preferences/PreferenceOverride'
import type { UserPreferences } from '../../types/preferences'
import type { PreferenceTracking, TripPreferenceMetadata } from '../../types/preferenceTracking'
import { getPreferenceSummary } from '../../types/preferenceTracking'
...
export interface TripFormData {
  ...
  preferenceTracking?: PreferenceTracking
}
...
export function TripPlanningForm({ onSubmit, onDataChange, initialData, isGenerating }: TripPlanningFormProps) {
  ...
  const [preferenceMetadata, setPreferenceMetadata] = useState<TripPreferenceMetadata | null>(null)
  ...
  const handleTrackingChange = useCallback((tracking: PreferenceTracking) => {
    setFormData(prevData => ({
      ...prevData,
      preferenceTracking: tracking
    }))
    
    // Calculate metadata
    const trackingValues = Object.values(tracking);
    const totalPreferences = trackingValues.length;
    const profilePreferences = trackingValues.filter(t => t.source === 'profile').length;
    const overriddenPreferences = trackingValues.filter(t => t.source === 'override').length;
    const defaultPreferences = trackingValues.filter(t => t.source === 'default').length;
    
    setPreferenceMetadata({
      totalPreferencesApplied: totalPreferences,
      profilePreferences,
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="mt-6">
          ...
          {/* Preference Summary */}
          {preferenceMetadata && preferenceMetadata.totalPreferencesApplied > 0 && (
            <div className="mt-3 flex items-center gap-2 text-xs text-gray-600 bg-gray-50 rounded-lg px-3 py-2">
              <Info className="w-3.5 h-3.5 text-gray-400" />
              <span>
                Using {preferenceMetadata.totalPreferencesApplied} preference{preferenceMetadata.totalPreferencesApplied !== 1 ? 's' : ''}:
                {' '}{getPreferenceSummary(preferenceMetadata)}
              </span>
            </div>
          )}
        </div>
